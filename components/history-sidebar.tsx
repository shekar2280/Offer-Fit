        ...
        <div className="w-[280px] bg-background/40 backdrop-blur-3xl border-r border-border/40 h-full flex flex-col shadow-[1px_0_20px_rgba(0,0,0,0.02)] z-10 shrink-0">
            ...
            <div className="p-5">
                <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl font-semibold transition-all duration-300 ${selectedId === null
            ...
            <div className="overflow-y-auto flex-1 px-3 pb-4 space-y-1 custom-scrollbar">
                ...
                {history.map((item) => (
                    <div
                        key={item.id} 
                        className="group/item relative flex items-center gap-2"
                    >
                        <button
                            onClick={() => onSelect(item.id)}
                            className={`flex-1 text-left px-3 py-3 rounded-xl transition-all duration-200 flex flex-col gap-1 relative overflow-hidden ${selectedId === item.id
                        ...
                        <button
                            ...
                            className="absolute right-2 opacity-0 group-hover/item:opacity-100 p-2 hover:bg-destructive/10 hover:text-destructive rounded-lg transition-all"
