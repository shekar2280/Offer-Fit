...
export function ResumeSetup({
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                            <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${hasCustomizeSource ? "text-primary" : "text-white"}`}>
                                {isLatex ? "LaTeX Source Loaded" : hasCustomizeSource ? "Resume Text Loaded" : "Upload LaTeX Source"}
                            </span>
                            <span className={`text-[9px] font-medium uppercase tracking-widest mt-1 ${hasCustomizeSource ? "text-white/20" : "text-primary/60"}`}>
                                {isLatex
                                    ...
                                    : hasCustomizeSource ? "Output will be plain text. Provide LaTeX for formatted code" : "Paste LaTeX code to start customizing"}
                            </span>
                        </div>
                    </div>
    ...
    const hasAnalysisSource = !!extractedText || !!latexText;

    return (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div className={`relative flex items-center justify-between w-full px-8 py-6 rounded-3xl border transition-all duration-700 ${hasAnalysisSource ? "bg-primary/[0.03] border-primary/20 shadow-[0_0_40px_rgba(242,170,76,0.05)]" : "bg-primary/[0.02] border-primary/30 shadow-[0_0_30px_rgba(242,170,76,0.03)]"}`}>
                <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-700 ${hasAnalysisSource ? "bg-primary text-black shadow-[0_0_20px_rgba(242,170,76,0.4)]" : "bg-primary/10 border border-primary/20 text-primary shadow-[0_0_15px_rgba(242,170,76,0.15)]"}`}>
                        {isLatex ? <Code2 className="w-5 h-5 opacity-100" /> : hasAnalysisSource ? <Check className="w-5 h-5 opacity-100" /> : <FileText className="w-5 h-5" />}
                    </div>
                    <div className="flex flex-col">
                        <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${hasAnalysisSource ? "text-primary" : "text-white"}`}>
                            {isLatex ? "LaTeX Source Loaded" : hasAnalysisSource ? "Resume Loaded" : "Upload Resume"}
                        </span>
                        <span className={`text-[9px] font-medium uppercase tracking-widest mt-1 ${hasAnalysisSource ? "text-white/20" : "text-primary/60"}`}>
                            {isLatex
                                ? `${charCount.toLocaleString()} chars · Resume ready for analysis`
                                : hasAnalysisSource ? "Your resume is ready for analysis" : "Upload PDF or paste LaTeX code to start"}
                        </span>
                    </div>
                </div>
                <div className="flex items-center gap-2 transition-all duration-300">
